# CLAUDE.md — Claude Code 用プロジェクト引継ぎノート

[重要] 本書は Claude Code（および互換ツール：Cursor / 他 AI ペアプロ）が **セッション再開時に最初に読むべき情報** をまとめる。  
理由: セッションや使用ツールが変わってもプロジェクト状態を素早く把握できるようにするため。
...
## 2. 現在地（2026-05-19 セッション終了時点）

### Git タグ
- **`v1.0.0-local`**：ローカル環境構築完了バージョン
- **`local-env-complete-20260519`**：マイルストーンタグ

### 進捗
- **未完了 15 / 81（約 81%）**
- 本日（2026-05-19）：**21 件完了退避 + 1 件不採用 + 3 件 007→020 移管 + 020 章新規 10 件追加 + 013 章新規 5 件全件完了**
- 第3段階セキュア化（ローカル）**完了**
...
| **011** 章 | 0 件 | 完了 |
| **012** 章 | 5 件 | **クラウド連携（AWS IoT Core）** ← 次の本線 |
| **013** 章 | 0 件 | 本日全件完了（文書整備） |
| **020** 章 | 13 件 | 将来対応 |
