...
[ツール切替時の注意] Cursor → Claude Code、または逆へツールを変える場合、`.cursorrules` と `CLAUDE.md` が同期されていることを確認する。差分があれば変更履歴の日付で新しい方を正とする。**最終整合確認日：2026-05-24（全 90 件完了・残 0/90）**。
...
## 2. 現在地（2026-05-24 セッション終了時点・全 90 件完了・残 0/90 🎉）
...
### 進捗
- **未完了 0 / 90（100% 完了 🎉）**
- 直近成果（2026-05-24）：**`010-0011` 完了**（CoreDNS ゾーン管理ルール §2.4.2：ローカル専用ゾーン限定・`fallthrough` 必須・公開ドメイン上書き禁止・現行 Corefile 文書化）。**`010-0012` 完了**（DHCP DNS 廃止計画 §2.4.3：ルータ DHCP DNS 設定手順・移行条件・移行タイミング定義）。**`012-0003` 完了**（`ドキュメント概要.md` ↔ `設計概要.md §3` 相互参照整合）。**`010-0014` 完了**（IoT LAN Firewall サブネット限定）。**`020-0022` 完了**（brokerMode 切替 UI）。**`020-0021` 完了**（NVS fallback IP）。
...
| **008** 章 | 0 件 | 全件退避済み |
| **009** 章 | 0 件 | 全件完了 |
| **010** 章 | 0 件 | 全件完了（`010-0011`/`010-0012` は 2026-05-24 完了・`010-0014` は 2026-05-24 完了） |
| **011** 章 | 0 件 | 完了 |
| **012** 章 | 0 件 | 全件完了（`012-0003` は 2026-05-24 完了・他は 2026-05-23 完了）→ `todo_old20260523.md` / `todo_old20260524.md` |
| **013** 章 | 0 件 | 全件完了（文書整備、2026-05-19） |
| **020** 章 | 10 件 | 将来対応。`020-0016`/`0017`/`0019`/`0020`/`0021`/`0022`/`0025` 完了退避済み（2026-05-24）。次: `020-0001`（本番鍵分離）等 |

### 実環境の現在状態（2026-05-24 セッション③終了時点）
- **ESP32**：local mode（`brokerMode=local`, `AP-IoTESP32Test`, IP=`172.17.1.200`）。FW=`1.1.0-beta.40`。LittleFS 更新済み（k-device=`7d1271a61762c6c0`）。MQTT ローカル通信確認 PASS。
- **LocalServer**：local mode（`CLOUD_MQTT_ENABLED=false`）。SecretCore 稼働中。
- **鍵保護方式**：ファイルベース AES-256-GCM（`LocalServer/data/keys/master_key.bin` + OS パーミッション）。旧 DPAPI 方式は退避済み。
- **Mosquitto**：稼働中
...
## 3. 次の本線（2026-05-24 時点）

### 次の本線選択肢（アクティブタスク 0 件・全 90 件完了）
1. **⚠️ 最優先（継続中）**: ローカル OTA テスト（`POST /api/ota/start`）→ OTA 完了確認
2. **Windows DPAPI 選択モード実装**（`PROTECT_MODE=dpapi` 環境変数）— 計画済み・未実装
3. **020 章将来対応**（`020-0001` 本番鍵分離 / `020-0011`〜`020-0014` 不可逆処理 / `020-0023`〜`020-0024` 等）を選択して着手
...
- 2026-05-24 (セッション③): DPAPI → ファイルベース AES-256-GCM 鍵保護移行完了（`key_protect.rs` + `data/keys/`）。ESP32 LittleFS uploadfs + MQTT ローカル通信確認 PASS。`applyIoTFirewallRules.ps1` 全英語化 + NIC 自動検出。LocalServer `k-user export` セキュリティ警告追加。`§2` 実環境状態・鍵保護方式を更新。次: ローカル OTA テスト + DPAPI 選択モード実装。
