...
[ツール切替時の注意] Cursor → Claude Code、または逆へツールを変える場合、`.cursorrules` と `CLAUDE.md` が同期されていることを確認する。差分があれば変更履歴の日付で新しい方を正とする。**最終整合確認日：2026-05-24 セッション④（全 90 件完了・残 0/90 🎉 + 020-0026/0027 新規追加）**。
...
## 2. 現在地（2026-05-24 セッション④終了時点・全 90 件完了・残 0/90 🎉）
...
### 進捗
- **未完了 0 / 90（100% 完了 🎉）**
- 直近成果（2026-05-24 セッション④）：**DPAPI 選択モード実装完了 + 両方式 MQTT/OTA E2E テスト PASS**。`key_protect.rs` デュアルモード（`PROTECT_MODE=dpapi` / デフォルト AES-256-GCM）。新規 todo `020-0026`/`020-0027` 追加。
- 直近成果（2026-05-24 セッション①〜③）：`010-0011`/`010-0012`/`012-0003`/`010-0014`/`020-0022`/`020-0021` 完了。DPAPI→ファイルベース AES-256-GCM 移行完了。
- 直近成果（2026-05-23）：**`012-0008` / `012-0009` 全件クリア・`012` 章完了**。S3 presigned URL 経由クラウド OTA・dual-partition 動作確認。タグ `v1.1.0-cloud-ota` 付与。

### 残タスク
| 章 | 件数 | 内容 |
|---|---|---|
| **007〜013** 章 | 0 件 | 全件完了・退避済み |
| **020** 章 | 12 件 | 将来対応。`020-0026`/`020-0027` は 2026-05-24 新規追加。次: `020-0026`（PROTECT_MODE 対話選択）/ `020-0001`（本番鍵分離）等 |

### 実環境の現在状態（2026-05-24 セッション④終了時点）
- **ESP32**：local mode（`brokerMode=local`, `AP-IoTESP32Test`, IP=`172.17.1.200`）。FW=`1.1.0-beta.40`、`runningPartition=1`（app1）。k-device=`9a57504897a75c5c`。
- **LocalServer**：local mode（`CLOUD_MQTT_ENABLED=false`, `PROTECT_MODE` 未設定 = ファイルベース）。SecretCore 稼働中（`data/keys/` 使用）。
- **鍵保護方式**：ファイルベース AES-256-GCM（`data/keys/master_key.bin`=32B + `wrapped_secret.bin`=280B）。DPAPI はオプション（`PROTECT_MODE=dpapi` で有効化可）。
- **Mosquitto**：稼働中 / **CoreDNS**：`Corefile bind 172.17.1.100` 限定
- **AWS**：無接続（課金なし）。S3 バケット空（ストレージ課金ゼロ）。
...
## 3. 次の本線（2026-05-24 セッション④終了時点）

### 次の本線選択肢（アクティブタスク 0 件・全 90 件完了）
1. **`020-0026`**: PROTECT_MODE 鍵作成時対話選択 UX（`.env` 手書き不要化、インタラクティブモード選択）
2. **`020-0001`**: 本番鍵分離（k-iot-secure-boot / k-iot-flash-encryption を本番専用に分離）
3. **`020-0011`/`020-0012`**: 不可逆処理 ProductionTool 実装（007-B No-Go 解除の前提）
4. **020 章その他将来対応**（`020-0004` Mac/Linux 汎用化 / `020-0007`〜`020-0010` / `020-0023`〜`020-0027`）
...
### セキュア設計の正本
- 鍵保護：**ファイルベース AES-256-GCM（デフォルト）** または **Windows DPAPI**（`PROTECT_MODE=dpapi`）。両方式は `key_protect.rs` でランタイム切替可能（2026-05-24 デュアルモード実装）。Mac/Linux 汎用化は `020-0004`
...
- 2026-05-24 (セッション④): **DPAPI 選択モード実装 + 両方式 E2E テスト完了**。`key_protect.rs` デュアルモード（ファイルベース AES-256-GCM / DPAPI）。両方式 MQTT+OTA PASS（partition 切替確認）。`020-0026`/`020-0027` 新規追加。`§2` 実環境状態・残タスク・次の本線を最終状態へ更新。セキュア設計の正本（鍵保護）を dual-mode に更新。コミット `260524-N`。
- 2026-05-24 (セッション③): DPAPI → ファイルベース AES-256-GCM 鍵保護移行完了（`key_protect.rs` + `data/keys/`）。ESP32 LittleFS uploadfs + MQTT ローカル通信確認 PASS。`applyIoTFirewallRules.ps1` 全英語化 + NIC 自動検出。次: ローカル OTA テスト + DPAPI 選択モード実装。
- 2026-05-24 (セッション①②): `010-0011` 完了（CoreDNS ゾーン管理ルール §2.4.2）。`010-0012` 完了（DHCP DNS 廃止計画 §2.4.3）。`012-0003` 完了（ドキュメント概要↔設計概要 相互参照）。`010-0014` 完了（IoT LAN Firewall）。`020-0022`/`020-0021`/`020-0020` 完了＋`020-0016/0017/0019/0025` 退避。**進捗 0/90（100% 完了 🎉）**。`§2` / `§3` を最終状態へ更新。
