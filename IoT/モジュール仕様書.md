...
- `IoT/LocalServer/`  
  ローカルHTTPサーバー実装、ローカル向けAPI提供
- `IoT/LocalMentenance/`  
  OTA・メンテナンス用GUI。サーバー連携機能を持つ保守ツール
- `IoT/LocalSoft/`  
  ESP32とMQTT通信し、状態監視・命令送信・一覧管理を行うローカル運用ソフト
- `IoT/CloudServer/`  
...
- ESPモジュール:
  ...
  - 実装言語: C/C++（[推奨] C++20）
- Localモジュール:
  - `LocalServer`: ローカルHTTPサーバーとしてESP32/LocalSoft/保守GUIと連携
  - `LocalMentenance`: OTA・メンテナンス実行GUI、保守操作の起点
  - `LocalSoft`: MQTT経由でESP32状態監視・命令送信・一覧可視化
