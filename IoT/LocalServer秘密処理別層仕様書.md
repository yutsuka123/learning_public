...
- [重要] `device_db` はバックアップ/復元対象として扱うが、サポートパッケージへは秘密項目を除いた要約のみを入れる。
- [厳守] サポートパッケージは `LocalServer` の画面状態、ログ要約、版数、対象機識別、エラー要約を中心に構成する。
- [推奨] 生成時は秘密項目の除外チェックを自動化し、手動チェックだけに依存しない。
