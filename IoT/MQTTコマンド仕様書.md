...
| `op` | `sub` | 方向 | 主用途 | 必須 `args` |
| :--- | :--- | :--- | :--- | :--- |
| `notice` | `trh` | ESP32 -> Server | 温湿度・気圧通知（TRH） | `temperatureC` `humidityRh` `pressureHpa` |
...
[重要] MQTTトピックは平文のまま維持し、payload本文は `k-device` で全文暗号化する。  
理由: 運用上のトピックルーティング性を維持しつつ、MQTTモニタ上で機密項目（args/設定値）を秘匿するため。

[厳守] 暗号化時のpayloadは以下のエンベロープ形式を使用する。
...
| キー | 型 | 必須 | 説明 |
| :--- | :--- | :--- | :--- |
| `v` | string | Yes | payload暗号化エンベロープ仕様のバージョン。 |
| `security.mode` | string | Yes | 暗号化方式識別子。現行は `k-device-a256gcm-v1` 固定。 |
| `enc.alg` | string | Yes | 暗号アルゴリズム。現行は `A256GCM` 固定。 |
| `enc.iv` | string | Yes | AES-GCM用IV（12byte）をBase64化した値。 |
| `enc.ct` | string | Yes | 平文JSON全体を暗号化したciphertextのBase64値。 |
| `enc.tag` | string | Yes | AES-GCM認証タグ（16byte）のBase64値。 |

- [厳守] `enc.alg` が `A256GCM` 以外、または `iv/ct/tag` 欠損時は復号せず `NG` とする。
- [厳守] `iv` の再利用を禁止する。
- [厳守] 復号失敗時に平文推定で処理を継続しない。

### 3.1.3 暗号化運用モード
[重要] 保守時の切替容易性と安全性を両立するため、payload暗号化モードを段階運用する。

| モード | 説明 | 主用途 |
| :--- | :--- | :--- |
| `plain` | 平文payloadのみ許可 | 障害切り分け（短時間） |
| `compat` | 暗号化優先、平文も受信許可 | 移行期間 |
| `strict` | 暗号化payloadのみ許可 | 本番運用 |

- [厳守] 本番運用では `strict` を目標とする。
- [新規利用禁止] 恒久的な `plain` 運用。
- [念のため保存] 移行手順として `compat` を残す。理由: 現地保守で段階切替が必要なため。
...
- [重要] `BME280` 利用時は `pressureHpa` を必須で送信する。
- [重要] `sensorAddress` は実機配線確認と障害切り分けのため `0x76` または `0x77` を文字列で含める。
...
### 3.3.6 コマンドリクエスト詳細: imagePackageApply（段階実装中）
...
- [進捗] 2026-03-12 時点で ESP32 は `signature` 検証、`destinationDir` スコープ検証、HTTPS取得、`packageSha256` 検証、ZIP展開、`imagePackageStatus` 通知まで実装済み。
- [制限] 現行実装は ZIP `stored`（圧縮方式 method=0）エントリのみ対応し、`deflate`（method=8）など他方式は拒否する。

### 3.3.7 通知詳細: imagePackageStatus
`imagePackageApply` 実行進捗を通知する。

**トピック**: `esp32lab/notice/imagePackageStatus/<senderName>`
...
```json
{
    ...
    "SrcID": "IoT_F0D0F94EB580",
    "Request": "Notice",
    "id": "IoT_F0D0F94EB580-123456",
    "sub": "imagePackageStatus",
    "sessionId": "imgpkg-20260310160000-001",
    "destinationDir": "/images/top",
    "phase": "downloading",
    "result": "OK",
...
- [厳守] `phase` は `planning` / `downloading` / `extracting` / `completed` を最低限許容する。
- [重要] `result=NG` の場合は `errorCode` を必須とする。
- [推奨] サーバー側は `sessionId` 単位で進捗集約し、最終フェーズ判定を実施する。
...
- 2026-03-12: payload全文暗号化エンベロープ（`security.mode` / `enc.alg` / `enc.iv` / `enc.ct` / `enc.tag`）と暗号化運用モード（`plain` / `compat` / `strict`）を追加。理由: LocalServer-ESP32間で `k-device` によるpayload暗号化を共通仕様として固定し、保守時の平文復帰手順も同時に明確化するため。
