...
### 5. 直近フェーズE: Production ツールと AP 画面分離の実装
- [ ] [2026-03-09][重要] `IoT/Production` を `LocalServer` と分離したメーカー専用ツール群として設計する。
  - 製造用書込み
  - 初回セキュア化
  - eFuse 最終有効化
  - 監査ログ保存
- [ ] [2026-03-09][重要] AP 共通トップ画面と Production 画面を分離する。
  - AP 名は共通
  - 共通トップ画面は共通パスワード認証
  - Production 操作は追加認証
  - 画面・権限・監査ログを分離
- [ ] [2026-03-09][厳守] 通常管理者パスワードとは別の「メーカーモード専用パスワード」の管理方式を設計する。
  - リポジトリ保存禁止
  - 変更・失効可能にする
  - 工場内での保管責任者、配布方法、更新手順を決める
- [ ] [2026-03-09][重要] Production 画面の段階実行ウィザードを設計する。
  - 事前確認
  - 対象機確認
  - 実行内容確認
  - 最終確認
...
- [ ] [2026-03-09][重要] ESP32 側に Production 専用セキュア化フローを設計する。
  - 通常OTA、通常アプリ、通常MQTT経路と共用しない
  - 事前条件確認、段階実行、各段階の eFuse 読戻し検証を入れる
- [ ] [2026-03-09][重要] Production 専用の事前チェック項目を確定する。
  - 対象機識別
  - 電源安定確認
  - 版数確認
  - 鍵ID確認
  ...
  - スタック余裕確認
  - 空きヒープ確認
- [ ] [2026-03-09][重要] Production 実行完了後の自動検証を設計する。
  - Secure Boot 有効確認
...
- [ ] [2026-03-09][重要] 7019〜7024 を実施し、Production 画面、再判定、スタック余裕、旧面継続起動まで `試験記録書.md` へ反映する。
- [ ] [2026-03-09][重要] eFuse・セキュアブート・ROM暗号化試験を実施し、OTA可否、FW制御によるシリアル出力制約、DL/UL可否を `試験記録書.md` へ記録する。

### 7. 直近フェーズG: eFuse最終化とOTA再更新
- [ ] [2026-03-09][重要] `Production` ツールから本番対象機へ eFuse 最終有効化を実施し、出荷可否判定ログを保存する。
- [ ] [2026-03-08][最重要] 7025 を実施し、eFuse 最終化後でも OTA によるバージョンアップができることを `試験記録書.md` へ反映する。

### 8. 直近ゴール後のセキュリティ拡張
- [ ] [2026-03-09][厳守] シリアル機能停止は eFuse ではなく FW 制御で管理する方針を実装へ反映する。
  - 量産通常FWでは高詳細ログと危険な保守コマンドを停止または最小化する
  - シリアル停止のみを目的に eFuse を焼かない
  - 問題解析時のみ署名済み診断用FWで一時的にシリアル出力を有効化できるようにする
- [ ] [2026-03-09][重要] 通常運用FWと診断用FWの差分方針を `機能仕様書.md` または専用仕様へ整理する。
  - 出力可能ログ種別
  - 許可する保守コマンド
  - 切替条件
  - 顧客環境で恒久運用しない条件
- [ ] [2026-03-09][重要] インターネット接続時の最低限セキュリティ基準を確定する。
  - 不要待受停止
  - 未使用ポート閉鎖
  - 管理ポート非公開
  - Topic ACL / API 認可
...
- [ ] [2026-03-09][重要] Mosquitto / LocalServer / ルータ / Firewall の各設定で、踏み台化防止の最低限対策を具体化する。
  - Broker 管理ポートの管理ネットワーク限定
  - 送信先制限
  - API 公開面の最小化
  - 認証失敗抑止
  - ログ保存先
- [ ] [2026-03-09][重要] `試験仕様書.md` の 7029〜7033 を実施し、シリアル制約、未使用ポート閉鎖、認証失敗抑止、踏み台化防止確認を `試験記録書.md` へ反映する。
...
- [ ] [2026-03-07][重要] SecretCore Phase1（Windows）を実装する。
  ...
  - zeroize によるメモリゼロ化
- [ ] [2026-03-07][重要] TS側 SecretCoreClient を実装する（IPC接続・暗号化通信・workflow 起動要求・進捗購読）。
- [ ] [2026-03-09][重要] `LocalServer` に高リスクワークフロー公開 API を実装する。
...
- [ ] [2026-03-09][重要] `SecretCore` の高リスクワークフロー実行基盤を実装する。
  - workflowId 付与
  - 状態管理
  - 進捗イベント配信
  - 完了判定
  - 監査ログ連携
- [ ] [2026-03-09][重要] 高リスクワークフローの正規進捗状態 `queued` / `running` / `waiting_device` / `verifying` / `completed` / `failed` を実装し、`LocalServer` UI へ表示する。
- [ ] [2026-03-09][重要] `LocalServer` 側の高リスク処理を、逐次オーケストレーション方式から「開始要求 + 進捗表示 + 結果表示」方式へ改修する。
  - `/api/workflows/...` 以外で高リスク処理を直公開しない
  - [禁止] `POST /api/pairing/bundle` のような内部 helper 直公開 API を作成しない
- [ ] [2026-03-09][重要] `SecretCore` に `runPairingSession()` を実装する。
  - `createPairingBundle` は内部ヘルパー化
  - 対ESP32通信開始
  - 保存完了確認
  - 完了判定
- [ ] [2026-03-09][重要] `SecretCore` に `runKeyRotationSession()` を実装する。
  - 新 `keyVersion` 生成
  - 個体単位再ペアリング
  - current/previous 更新確認
  - 完了判定
- [ ] [2026-03-09][重要] `SecretCore` に `runProductionSecureFlow()` を実装する。
  - 事前条件確認
  - 段階実行
  - 読戻し検証
  - 隔離判定
- [ ] [2026-03-09][重要] `SecretCore` に `runSignedOtaCommand()` を実装する。
  - 署名付き OTA 開始指示
  - 送達確認
  - 検証結果確認
  - 完了判定
- [ ] [2026-03-09][重要] `SecretCore` に `createPairingBundle` 内部 helper を実装する。
  - `targetDeviceId`
  - `sessionId`
  - `keyVersion`
  - `requestedSettings`
...
- [ ] [2026-03-09][重要] AP 共通トップ画面の共通パスワード認証と、Production 追加認証を実装する。
  - 共通トップ画面は全デバイス共通固定値で開始
  - 将来変更可能な設計余地を残す
  - Production はメーカーモード専用パスワードで追加認証
  - 10分ごと高権限再認証
  - 3回失敗時は AP 再接続または5分待機
- [ ] [2026-03-09][重要] AP 共通トップ画面へ `shortId`、`public_id`、Production対応/非対応、eFuse、Secure Boot、Flash Encryption、シリアル状態を表示する。
...
- [ ] [2026-03-07][推奨] SecretCore Phase4（完全化）を実装する。
  ...
  - HMACチェーン付き監査ログ
  - `k-device` 導出とペアリング連携
- [ ] [2026-03-09][重要] 高リスクワークフローの結合試験を `試験仕様書.md` へ追加する。
  - `runPairingSession()`
...
- [ ] [2026-03-09][重要] `public_id` 初期値を `IoT_<macアドレスからコロン除去>` とする仕様変更を ESP/サーバ双方へ反映する。
...
- [ ] [2026-03-09][重要] ペアリング手順（`runPairingSession()` -> ECDH -> `createPairingBundle` 内部生成 -> `k-device` / Wi-Fi / MQTT / OTA / 認証情報 の NVS 保存）を実装する。
- [ ] [2026-03-09][重要] ESP32 の `current/previous` key 2 スロット運用と稼働時間 48h / 240h 管理を実装する。
