...
function demonstrateBasicTypes() {
    ...
    // 文字列型
    const userName = '田中太郎';
    const userMessage = `こんにちは、${userName}さん！`;
    // 数値型
    const userAge = 25;
    const userScore = 98.5;
    const hexValue = 0xFF;
...
function demonstrateArrayTypes() {
    ...
    const readonlyFruits = ['りんご', 'バナナ'];
    // タプル型（固定長で異なる型を持つ配列）
    const person = ['田中太郎', 30];
    const coordinate = [10, 20];
    const userInfo = ['佐藤花子', 25, true];
    // オプショナル要素を持つタプル
    const optionalTuple = ['テスト'];
    const optionalTuple2 = ['テスト', 42];
    // 残余要素を持つタプル
    const restTuple = ['ラベル', 1, 2, 3, 4];
...
function demonstrateUnionAndIntersectionTypes() {
    ...
    const mixedValue1 = 'テキスト';
    const mixedValue2 = 42;
    const nullableBoolean = true;
    // Intersection型の使用例
...
function demonstrateFunctionTypes() {
    ...
    // オプショナル引数とデフォルト引数
    function greet(name, greeting = 'こんにちは', punctuation) {
 ...
 */
async function runWithFrontendIntegration() {
    try {
...
//# sourceMappingURL=01_basic_types_practice.js.map