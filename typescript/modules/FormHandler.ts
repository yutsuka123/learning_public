...
export class ValidationRules {
     ...
     */
    public static required(fieldName: string): ValidationRule {
     ...
     * @returns バリデーションルール関数
     */
    public static minLength(minLength: number, fieldName: string): ValidationRule {
        return (value: string): ValidationResult => {
     ...
     * @returns バリデーションルール関数
     */
    public static maxLength(maxLength: number, fieldName: string): ValidationRule {
        return (value: string): ValidationResult => {
     ...
     * - サーバーサイドでの最終検証が必須
     */
    public static email(fieldName: string): ValidationRule {
        return (value: string): ValidationResult => {
...
export class FormHandler {
    ...
    private readonly fields: ReadonlyArray<FormFieldConfig>;
    private readonly submitCallback: (data: FormData) => Promise<boolean>;
    ...
    constructor(
        ...
        submitCallback: (data: FormData) => Promise<boolean>,
        errorContainerSelector: string = '.error-container'
    ) {
        // フォーム要素の取得と検証
        const formResult = DOMUtils.querySelector<HTMLFormElement>(formSelector);
        ...
        // エラーコンテナの取得と検証
        const errorResult = DOMUtils.querySelector<HTMLElement>(errorContainerSelector);
    ...
    private setupEventListeners(): void {
        // フォーム送信イベントの設定
        const submitResult = DOMUtils.addEventListener(
            this.formElement,
            'submit',
        ...
        this.fields.forEach(field => {
            if (field.realTimeValidation) {
                const inputResult = DOMUtils.querySelector<HTMLInputElement>(
                    `[name="${field.name}"]`,
                    this.formElement
                );
     ...
     */
    private validateAllFields(): FormSubmitResult {
     ...
     */
    private displayFieldError(fieldName: string, message: string): void {
        const errorElementResult = DOMUtils.querySelector<HTMLElement>(
     ...
     * @param errors - エラー配列
     */
    private displayErrors(errors: ReadonlyArray<ValidationResult>): void {
    ...
    private setLoadingState(loading: boolean): void {
        const submitButtonResult = DOMUtils.querySelector<HTMLButtonElement>(
            'button[type="submit"]',
            this.formElement
        );
