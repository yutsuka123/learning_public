...
export class FrontendApp {
    ...
    constructor(config: AppConfig) {
        ...
        // コンテナ要素の取得
        const containerResult = DOMUtils.querySelector<HTMLElement>(config.containerSelector);
     ...
     * - レスポンシブデザイン
     */
    private async buildUIStructure(): Promise<void> {
        const uiTemplate = `
    ...
    private setupResetButton(): void {
        const resetBtnResult = DOMUtils.querySelector<HTMLButtonElement>('#reset-btn');
    ...
    private setupTooltips(): void {
        const tooltipTargets = [
            { selector: '.required', text: 'この項目は必須です', position: 'top' as const },
            { selector: '#total-users', text: '登録されたユーザーの総数', position: 'bottom' as const },
            { selector: '#average-age', text: '全ユーザーの平均年齢', position: 'bottom' as const }
        ];

        tooltipTargets.forEach(({ selector, text, position }) => {
            const elementsResult = DOMUtils.querySelectorAll<HTMLElement>(selector);
     ...
     * @returns 送信成功フラグ
     */
    private async handleFormSubmit(formData: Record<string, string>): Promise<boolean> {
        try {
     ...
     * @param user - ユーザー情報
     */
    private addUserCard(user: UserProfile): void {
        const card = new Card({
            title: user.name,
            content: `
        ...
        // グリッドに追加
        const gridResult = DOMUtils.querySelector<HTMLElement>('#users-grid');
    ...
    private performDeleteUser(userId: string): void {
        ...
        // UIから削除
        const cardIndex = this.userCards.findIndex(card => 
            card.getElement().querySelector('.card-title')?.textContent === 
    ...
    private updateStatistics(): void {
        ...
        // 総ユーザー数
        const totalUsersResult = DOMUtils.querySelector<HTMLElement>('#total-users');
        ...
        if (users.length > 0) {
            ...
            const averageAgeResult = DOMUtils.querySelector<HTMLElement>('#average-age');
            ...
            const latestUserResult = DOMUtils.querySelector<HTMLElement>('#latest-user');
            ...
            if (averageAgeResult.success) averageAgeResult.data.textContent = '0';
            if (latestUserResult.success) latestUserResult.data.textContent = '-';
        }
    }
